...
namespace Ident {
    ...
    inline pair<Ident_T, IdentBase_T> split_last(Ident_T x) {
        AT(x->xs_->length() >= 1);
        auto xs_new = make_rc<Vec<IdentBase_T>>();
        for (Int i = 0; i < x->xs_->length() - 1; i++) {
            xs_new->push_back(x->xs_->operator[](i));
        ...
        return make_pair(ret_proper, x->xs_->operator[](x->xs_->length() - 1));
    }
    ...
    inline bool starts_with(Ident_T x, Ident_T prefix) {
        ...
        for (Int i = 0; i < prefix->xs_->length(); i++) {
            if (val_hash(x->xs_->operator[](i)) != val_hash(prefix->xs_->operator[](i))) {
                return false;
            }
        }
        return true;
    ...
    inline Ident_T empty() {
        return Ident::make(make_rc<Vec<IdentBase_T>>());
    }
}
...
inline IdentBase_T ident_base_from_string(string s) {
    return IdentBase::User::make(s);
}


inline Ident_T ident_singleton_from_string(string s) {
...
inline void pr(ostream& os, FmtFlags flags, IdentBase_T x) {
    if (x->is_User()) {
...
inline void pr(ostream& os, FmtFlags flags, Ident_T x) {
    if (x->xs_->length() == 0) {
        os << "<empty_id>";
        return;
    }
...
inline void pr(ostream& os, FmtFlags flags, GenType_T x) {
    pr_debug(os, flags, x);
}


inline void pr(ostream& os, FmtFlags flags, WriteInstr_T x) {
    pr_debug(os, flags, x);
}


inline void pr(ostream& os, FmtFlags flags, UnwindInstr_T x) {
    pr_debug(os, flags, x);
}


inline void pr(ostream& os, FmtFlags flags, ParseExpr_Base_T x) {
    if (x->src_.is_none()) {
...
inline string str_short(Sym_T x) {
    if (x->is_Term()) {
        return fmt_str("{}", x);
    } else if (x->is_Start()) {
        return fmt_str("{}", x);
...
inline void pr(ostream& os, FmtFlags flags, Sym_T x) {
    if (x->is_Term()) {
...
inline void pr(ostream& os, FmtFlags flags, ProdId_T x) {
    if (x->is_Start()) {
        fmt(os, "ProdId(&&.{})", x->as_Start()->sym_);
    } else if (x->is_Explicit()) {
        fmt(os, "ProdId({}.{})", x->as_Explicit()->rule_ind_, x->as_Explicit()->sub_);
    } else if (x->is_GenCPS()) {
        auto cc = x->as_GenCPS();
        if (cc->uniq_ind_ == 0) {
            fmt(os, "{}", cc->orig_);
...
inline void pr(ostream& os, FmtFlags flags, Prod_T prod) {
    string ret = fmt_str("{} -> ", str_short(prod->lhs_));
...
inline void pr(ostream& os, FmtFlags flags, DottedProd_T prod) {
    string ret = fmt_str("{} -> ", str_short(prod->prod_->lhs_));
...
inline void pr(ostream& os, FmtFlags flags, AttrKey_T x) {
    string suffix = "";
    if (x->loc_.is_some()) {
...
inline void pr(ostream& os, FmtFlags flags, AttrType_T x) {
    if (x->is_Bool()) {
        fmt(os, "bool");
    } else if (x->is_Range()) {
...
inline void pr(ostream& os, FmtFlags flags, ProdConstrRhsLoc loc) {
    if (loc == ProdConstrRhsLoc::All) {
        fmt(os, "rhs");
    } else if (loc == ProdConstrRhsLoc::Begin) {
        fmt(os, "rhs_begin");
...
inline void pr(ostream& os, FmtFlags flags, ProdConstr_T ce) {
    if (ce->is_LhsLeq()) {
        ...
        if (ce->as_Implies()->kl_->is_Id()) {
            fmt(os, "lhs[{}] -> {}[{}]",
                ce->as_Implies()->kl_, ce->as_Implies()->loc_, ce->as_Implies()->kr_);
        ...
        if (ce->as_RhsGeq()->v_->is_Bool()) {
            ...
            fmt(os, "{}[{}] >= {}",
                ce->as_RhsGeq()->loc_, ce->as_RhsGeq()->k_, ce->as_RhsGeq()->v_);
        } else {
            AX();
        }
    } else {
...
inline void pr(ostream& os, FmtFlags flags, ProdConstrFlat_T ce) {
    if (ce->is_LhsLeq()) {
        if (ce->as_LhsLeq()->v_->is_Bool()) {
            fmt(os, "!lhs[{}]", ce->as_LhsLeq()->k_);
        } else if (ce->as_LhsLeq()->v_->is_Int_()) {
        ...
        if (ce->as_Implies()->kl_->is_Id()) {
            fmt(os, "lhs[{}] -> rhs.{}[{}]",
                ce->as_Implies()->kl_, ce->as_Implies()->i_, ce->as_Implies()->kr_);
        } else if (ce->as_Implies()->kl_->is_Prec()) {
            fmt(os, "lhs[{}] <= rhs.{}[{}]",
                ce->as_Implies()->kl_, ce->as_Implies()->i_, ce->as_Implies()->kr_);
        } else {
        ...
        if (ce->as_RhsGeq()->v_->is_Bool()) {
            ...
            fmt(os, "rhs.{}[{}] >= {}",
                ce->as_RhsGeq()->i_, ce->as_RhsGeq()->k_, ce->as_RhsGeq()->v_);
        } else {
            AX();
        }
    } else {
...
inline void pr(ostream& os, FmtFlags flags, LRSym_T x) {
    if (x->is_Base()) {
...
inline void pr(ostream& os, FmtFlags flags, SymStr_T x) {
    if (x->v_->length() == 0) {
        fmt(os, "eps");
        return;
    }
...
inline void pr(ostream& os, FmtFlags flags, StringSet_T<SymStr_T> x) {
    fmt(os, "{{");
    bool fresh = true;
...
inline void pr(ostream& os, FmtFlags flags, StringSet_T<Unit> x) {
    fmt(os, "{{");
    bool fresh = true;
    for (auto si : *x->items_) {
        if (!fresh) {
...
inline void pr(ostream& os, FmtFlags flags, LRLabel_T x) {
    if (x->is_Eps()) {
        fmt(os, "eps");
    } else if (x->is_Sym_()) {
        string attr_str = "";
...
inline void pr(ostream& os, FmtFlags flags, LRAction_T x) {
    if (x->is_Shift()) {
        fmt(os, "Shift");
    } else if (x->is_Reduce()) {
...
inline void pr(ostream& os, FmtFlags flags, LRLookAction_T x) {
    fmt(os, "{} -> {}", x->la_, x->acc_);
}


inline void pr(ostream& os, FmtFlags flags, LRVertex_T x) {
...
inline void pr(ostream& os, FmtFlags flags, AttrVal_T x) {
    if (x->is_Bool()) {
...
inline void pr(ostream& os, FmtFlags flags, AttrSet_T x) {
    bool fresh = true;
    for (auto xi : *x->m_) {
        if (!fresh) {
            fmt(os, ",");
...
inline void pr(ostream& os, FmtFlags flags, AttrBoundSet_T x) {
    bool fresh = true;
    for (auto xi : *x->m_) {
        if (!fresh) {
            fmt(os, ",");
...
inline void pr(ostream& os, FmtFlags flags, LangCompileResult::Error_T x) {
    if (x->is_LRConf()) {
        Int conf_i = 0;
...
inline void pr(ostream& os, FmtFlags flags, SwitchTable_T x) {
    pr_debug(os, flags, x);
}
...
inline bool dotted_prod_is_start(DottedProd_T dp) {
    return dp->dot_ == 0;
}
...
inline LRVertex_T lr_vertex_without_lookahead(LRVertex_T v) {
    ...
    if (v->is_Prod()) {
        return LRVertex::Prod::make(
            v->as_Prod()->bounds_, la_trivial, v->as_Prod()->prod_, v->as_Prod()->left_rec_);
    } else if (v->is_RecStart()) {
        return LRVertex::RecStart::make(
            v->as_RecStart()->bounds_, la_trivial, v->as_RecStart()->sym_);
    } else if (v->is_RecEnd()) {
        return LRVertex::RecEnd::make(
            v->as_RecEnd()->bounds_, la_trivial, v->as_RecEnd()->sym_);
    } else {
        AX();
    }
}
...
inline void pr(ostream& os, FmtFlags flags, LRStringExemplar_T x) {
    fmt(os, "{}=[{}]", LRLabel::Sym_::make(x->sym_, x->attr_), x->contents_);
}


inline void pr(ostream& os, FmtFlags flags, LRStringExemplarBounded_T x) {
...
inline Unit string_set_repr_concat(Unit x, Unit y) {
    return Unit{};
}
...
inline data::Node_T data_gen_sum_id_to_node(DataGenContext data, Ident_T id) {
    ...
    return data.Q_->qq_ext(Some<string>("SumId"), *ret_items);
}
