...
inline NameMaybe_T with_sub(NameMaybe_T nm, NameMaybeBase_T item,
                            Option_T<IdentBase_T> item_default,
...
void parser_Gr_flat_add_prod(LangCompileContext &ctx, const Sym_T &lhs,
  ...
  auto [prod, is_new] =
      Grammar::add_prod(ctx.Gr_flat_, lhs, rhs_sym, prod_id, rhs_unfold);
  AT(is_new);
...
inline SymFlattenResult_T parser_resolve_base(LangCompileContext &ctx,
                                              ParseExpr_Base_T xb) {
  ...
  if (!is_term && !is_nonterm) {
    ctx.error(xb->src_.as_some(), "Leaf expression not found (must be either a "
                                  "parser rule or a top-level lexer token)");
  }
  ...
  if (is_term) {
    ...
    bool unfold = false;
    return SymFlattenResult::make(term_sym, unfold,
                                  attr_leaf_lift_expr(xb->src_));
  } else if (is_nonterm) {
    ...
    bool unfold =
        ctx.parse_expr_props_->operator[](xb->src_.as_some()->id_)->unfold_;
...
inline NameMaybeBase_T parser_flatten_lookup_name(ParseExpr_T x,
                                                  LangCompileContext &ctx) {
...
parser_flatten_expr_new(ParseExpr_T x, const NameMaybe_T &id, bool owns_id,
  ...
  // Optimization: Avoid generating an additional nonterminal if we are at a
  // base case.
  ...
  if (xbm.is_some()) {
    ...
    auto [r_sym, r_fl] =
        parser_flatten_gen_nonterm(ctx, Sym::Direct::make(x, id_gensym),
...
inline void parse_expr_squash_concat_rec_acc(Vec_T<ParseExpr_T> &dst,
                                             ParseExpr_T x,
                                             LangCompileContext &ctx) {
...
inline std::pair<bool, bool> parser_flatten_expr_concat_acc_extract_spec(
    Vec_T<ParseExpr_T> ys, bool allow_passthrough, LangCompileContext &ctx);

// Conservative approximation
inline bool parser_flatten_predict_is_dt_conservative(ParseExpr_T x,
                                                      LangCompileContext &ctx) {
...
inline std::pair<bool, bool> parser_flatten_expr_concat_acc_extract_spec(
  ...
  if (nontrivial_inds->length() == 1) {
    ...
    if (!allow_passthrough &&
        !parser_flatten_predict_is_dt_conservative(yi_nt, ctx)) {
      is_singleton = false;
    }
  ...
  return std::make_pair(is_empty, is_singleton);
}
...
parser_flatten_expr_alt_acc_new(ParseExpr_T x, const Sym_T &dst,
  ...
  for (Int i = 0; i < ys->length(); i++) {
    ...
    auto id_gensym = ctx.Gr_flat_nonterm_gensym_->gen();
    auto [ri_sym, ri_fl] =
        parser_flatten_gen_nonterm(ctx, Sym::Direct::make(yi, id_gensym),
                                   rule_ctx, true, Some<ParseExpr_T>(yi));
    ...
    ctx.gen_dt_parent_mapping_->insert(id_sub_yi_f,
                                       std::make_pair(id_f, yi_nm_f));
  }
...
parser_flatten_expr_concat_acc_new(
  ...
  for (Int i = 0; i < ys->length(); i++) {
    ...
    if (!is_singleton) {
      id_sub_yi = NameMaybe::with_sub(id, parser_flatten_lookup_name(yi, ctx),
                                      yi_nm_default, ctx);
    } else {
      owns_id_sub = false;
    }

    auto [yi_fl, yi_wr, yi_ty, yi_is_dt] =
        parser_flatten_expr_new(yi, id_sub_yi, owns_id_sub, rule_ctx, ctx);
  ...
  if (is_empty) {
    ...
    rd_instr = UnwindInstr::SelectIndex::make(
        ret_i_nonempty_singleton.as_some(), rhs_tys);
    ...
    auto ret_dt =
        GenDatatype::make(ret_dt_items, None<Map_T<IdentBase_T, GenType_T>>());
...
parser_flatten_expr_iter_acc_new(ParseExpr_T x, const Sym_T &dst,
  ...
  auto id_sub_y =
      NameMaybe::with_sub(id, parser_flatten_lookup_name(y, ctx),
                          Some<IdentBase_T>(IdentBase::ItemInner::make()), ctx);

  auto rd_instr_id = UnwindInstr::Identity::make();

  auto [y_fl, y_wr, y_ty, y_is_dt] =
      parser_flatten_expr_new(y, id_sub_y, true, rule_ctx, ctx);
  ...
  if (x->is_Rep() || x->is_RepNonzero()) {
    ...
    // yl -> y yl
    {
      ...
      rhs_fl_v->push_back(yl_fl);
      parser_Gr_flat_add_prod(ctx, yl_sym, None<AttrLeaf_T>(), rhs_fl_v,
                              rule_ctx, UnwindInstr::VecCons::make(id_f, y_ty));
    }
    ...
    ret_wr = WriteInstr::RecList::make(y_wr, ListFormat::Inline::make(),
                                       None<WriteInstr_T>(),
                                       HasFinalDelim::N::make());
    ...
    if (parse_expr_is_parse_phase_empty_base(xc->delim_)) {
      ...
      auto [delim_fl, delim_wr, delim_ty, delim_is_dt] =
          parser_flatten_expr_new(delim, id_sub_delim, true, rule_ctx, ctx);
      ...
      if (n_min == 2) {
        // dst -> y d yl
        {
          ...
          parser_Gr_flat_add_prod(
              ctx, dst, lhs_leaf, rhs_fl_v, rule_ctx,
...
parser_flatten_expr_acc_new(ParseExpr_T x, const Sym_T &dst,
  ...
  if (x->is_Alt()) {
    return parser_flatten_expr_alt_acc_new(x, dst, lhs_leaf, id, owns_id,
                                           rule_ctx, ctx);

  } else if (x->is_Optional()) {
    return parser_flatten_expr_optional_acc_new(x, dst, lhs_leaf, id, owns_id,
                                                rule_ctx, ctx);
    ...
    return parser_flatten_expr_iter_acc_new(x, dst, lhs_leaf, id, owns_id,
                                            rule_ctx, ctx);

  } else if (x->is_Pass() || x->is_Underscore()) {
    auto rd_instr = UnwindInstr::Nil::make(make_rc<Vec<GenType_T>>());
    parser_Gr_flat_add_prod(ctx, dst, lhs_leaf,
                            make_rc<Vec<SymFlattenResult_T>>(), rule_ctx,
                            rd_instr);
    ...
    auto [x_fl, x_wr, x_ty, x_is_dt] =
        parser_flatten_expr_base_new(xb, id, rule_ctx, ctx);
...
inline void
parser_flatten_grammar_rule_acc(LangCompileContext &ctx,
                                meta::Node::ParserDecl::Rule_T rule) {
...
parser_flatten_gen_inline_sum_cases(const Ident_T &id,
  ...
  auto dt = GenDatatype::make(dt_fields,
                              Some<Map_T<IdentBase_T, GenType_T>>(dt_cases));
...
void parser_tabulate_syms_top(LangCompileContext &ctx) {
  ...
  for (const auto &sym_top : *ctx.parser_syms_top_) {
    ...
    if (!ctx.Nm_->contains_key(sym_top_base)) {
      ctx.error(
          ctx.parser_,
          fmt_str("Definition not found for top-level parseable symbol: {}",
                  sym_top_base));
    }
    ...
    auto [prod, is_new] = Grammar::add_prod(ctx.Gr_flat_, start, prod_rhs,
                                            prod_id, prod_unfold_mask);
    AT(is_new);
...
void parser_flatten_grammar_rules(LangCompileContext &ctx) {
  ...
  for (const auto &p : *ctx.parser_rule_inds_) {
    ...
    for (const auto &name : *rule_id->xs_) {
      ...
      ctx.gen_dt_parent_mapping_->insert(id_acc_sub,
                                         std::make_pair(id_acc, name));
  ...
  for (const auto &[id, ty] : *ctx.gen_dt_map_) {
    if (ty->is_Datatype() && !ctx.gen_dt_parent_mapping_->contains_key(id)) {
      ...
      ctx.gen_dt_parent_mapping_->insert(id,
                                         std::make_pair(id_top, child_id_flat));
