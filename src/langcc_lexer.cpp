...
inline void pr(std::ostream &os, FmtFlags /*flags*/, const Lexer_NFAAcc &x) {
...
void lang_init_validate_tabulate_lexer_instr_emit_rec(
  ...
  if (instr->is_Emit() || instr->is_PopEmit()) {
    ...
    for (const auto &emit_tok : emit_toks) {
      if (emit_tok->tok_->is_Id() || emit_tok->tok_->is_LitStr()) {
        ...
        ctx.error(instr, "Emitted token must be id or std::string literal");
      }
    }
    ...
    for (const auto &case_sub : *instr->as_MatchHistory()->cases_) {
...
void lexer_check_all_reach_visit_instr(lang::meta::Node::LexerInstr_T instr,
                                       Map_T<Ident_T, ParseExpr_T> &id_reach,
                                       Vec_T<Ident_T> &Q) {
...
lexer_check_all_reach(LangCompileContext &ctx) {
  ...
  for (const auto &mode : ctx.lexer_modes_) {
    for (const auto &case_ : *mode->cases_) {
      if (case_->tok_->is_Id()) {
        ...
        for (const auto &x : *case_->tok_->as_Alt()->xs_) {
          if (x->is_Id()) {
            auto x_id = parse_expr_id_to_ident(x->as_Id()->id__);
            if (!id_reach->contains_key(x_id)) {
              id_reach->insert(x_id, x);
  ...
  while (!Q->empty()) {
    ...
    if (!ctx.tokens_def_.contains_key(curr)) {
      ctx.error(id_reach->operator[](curr),
                "Identifier not found in lexer tokens");
    }
    visit_lang_meta_Node(ctx.tokens_def_[curr], [&](lang::meta::Node_T sub) {
      if (sub->is_ParseExpr() && sub->as_ParseExpr()->is_Id()) {
        auto sub_id =
            parse_expr_id_to_ident(sub->as_ParseExpr()->as_Id()->id__);
  ...
  while (!Q.empty()) {
    ...
    if (ctx.tokens_top_by_id_rev_.contains_key(tok)) {
      if (!vis.contains(ctx.tokens_top_by_id_rev_[tok])) {
        ...
        Q.push_back(tok);
        auto tok_ind =
            Lexer_TopTokenInd::make(ctx.tokens_top_by_id_rev_[tok], tok);
...
template <typename N>
void lexer_nfa_item_compile(meta::Node::ParseExpr_T src, N &nfa,
                            Lexer_NFAVertex v_src, Lexer_NFAVertex v_dst,
                            Option_T<meta::Node::ParseExpr_T> src_wildcard,
                            LangCompileContext &ctx) {
  ...
  if (src->is_Alt()) {
    ...
    lexer_nfa_item_compile(src->as_AltExplicit()->e_, nfa, v_src, v_dst,
                           src_wildcard, ctx);
    ...
    auto v_curr = v_src;
    for (Int i = 0; i < string_to_int(cc->count_.to_std_string()).as_some();
         i++) {
      auto v_next = NFA::gen_vertex(nfa);
    ...
    lexer_nfa_item_compile(src_wildcard.as_some(), nfa, v_src, v_dst,
                           src_wildcard, ctx);
    ...
    lexer_nfa_item_compile(src->as_Paren()->x_, nfa, v_src, v_dst, src_wildcard,
                           ctx);
...
LexerNFASub_T lexer_nfa_compile_sub(const meta::Node::ParseExpr_T &x,
  ...
  lexer_nfa_item_compile(x, nfa_sub, start_sub, end_sub,
                         None<meta::Node::ParseExpr_T>(), ctx);
...
void lexer_step_exec_compile_instr_acc(Vec_T<cc::Node_T> &dst,
  ...
  if (instr->is_Emit()) {
    ...
    dst->push_back(cc.qq("Stmt", "st->enqueue_emit_ext(emit_dst, ws_state, ",
    ...
    for (Int i = 0; i < ic->cases_->length(); i++) {
      ...
      if (arg_ids.is_none()) {
        ...
        for (const auto &arg_id : *arg_ids.as_some()) {
    ...
    if (!has_default) {
      cc_cases->push_back(cc.qq_switch_case(
          "default: { st->error_here_match_history(in_i); }"));
    }
  ...
  for (const auto &mode : ctx.lexer_modes_) {
    ...
    for (Int case_id = 0; case_id < mode->cases_->length(); case_id++) {
      ...
      if (pat->is_Underscore()) {
        ...
        auto w = meta::Node::ParseExpr::Minus::make(
            ctx.gen_meta_->gen(), bounds, false, -1, attr_mask_trivial(),
            parser_lookahead_trivial(),
      ...
      if (lexer_instrs_has_emit_match(case_->instrs_)) {
        ...
        for (const auto &emit_id : *emit_ids) {
          ...
          lexer_nfa_item_compile(emit_expr->src_.as_some(), nfa, v_src_curr,
                                 v_dst, src_wildcard, ctx);
        }
      } else {
    ...
    if (NFA::has_conflicts(dfa_res)) {
      for (const auto &vs : *dfa_res->G_->V_) {
        ...
        if (acc_i->length() > 1) {
          ...
          Q->push_back(v_start);
          auto m =
              make_rc<Map<Set_T<Int>,
                          Option_T<std::pair<Lexer_NFALabel_T, Set_T<Int>>>>>();
          m->insert(v_start, None<std::pair<Lexer_NFALabel_T, Set_T<Int>>>());
          while (!Q->empty()) {
            auto curr = Q->pop_front_val();
          ...
          LangCompileResult::Error_T err =
              LangCompileResult::Error::Other::make(
...
lexer_extract_trivial_maybe(meta::Node::LexerDecl::Mode_T mode) {
  ...
  bool has_default = false;
  ret.cases_ = make_rc<Vec<std::pair<Option_T<Vec_T<Ch>>, LexerInstrs>>>();
  ...
  for (const auto &case_ : *mode->cases_) {
    ...
    ret.cases_->push_back(std::make_pair(cs, case_->instrs_));
  }
