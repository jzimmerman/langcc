...
struct DataCompileContext {
  ...
  Map_T<GenName, std::pair<GenName, IdBase>> sum_cases_rev_;
  ...
  Vec_T<std::string> includes_;

  inline DataCompileContext() {
    sum_cases_ = make_rc<Map<GenName, Map_T<IdBase, GenName>>>();
    sum_cases_rev_ = make_rc<Map<GenName, std::pair<GenName, IdBase>>>();
  ...
  inline Vec_T<std::pair<GenName, Option_T<IdBase>>>
  dt_ancestors(const GenName &dt) {
    GenName dt_curr = dt;
    auto ret = make_rc<Vec<std::pair<GenName, Option_T<IdBase>>>>();
    ...
    while (true) {
      ...
      ret->push(std::make_pair(dt_curr, sel));
...
void tabulate_data_decl_acc(data::Node_T src, const GenName &ns,
                            DataCompileContext &ctx) {
  ...
  if (decl->is_Include()) {
    ...
    if (dd->base_.is_some()) {
      dd_name_full_sum =
          ns->with_extend(data_id_to_name(dd->base_.as_some()->items_));
    ...
    if (is_sum_case) {
      if (!ctx.sum_cases_->contains_key(dd_name_full_sum)) {
        ctx.sum_cases_->insert_strict(dd_name_full_sum->clone_rc(),
                                      make_rc<Map<IdBase, GenName>>());
      }
      ...
      ctx.sum_cases_rev_->insert_strict(
          ...
          std::make_pair(dd_name_full_sum->clone_rc(), dd_name_full_base));
    }
...
Option_T<std::pair<GenName, bool>>
ns_resolve(const GenName &ns, const GenName &name, DataCompileContext &ctx) {

  for (Int i = ns->length(); i >= 0; i--) {
    auto name_cand = ns->slice(0, i)->with_extend(name);
    if (ctx.sum_cases_->contains_key(name_cand)) {
      return Some(std::make_pair(name_cand, true));
    } else if (ctx.enum_leaves_->contains_key(name_cand)) {
      return Some(std::make_pair(name_cand, false));
    } else if (ctx.data_leaves_->contains_key(name_cand)) {
      auto is_indirect = true;
      return Some(std::make_pair(name_cand, is_indirect));
    } else if (name->length() == 1 && ctx.param_src_->contains_key(name_cand)) {
      auto name_s = make_rc<Vec<std::string>>();
      name_s->push_back(name->only());
      return Some(std::make_pair(name_s, false));
    }
  }

  return None<std::pair<GenName, bool>>();
}

inline IdBase name_sep_to_join(const GenName &name, const std::string &delim) {
  bool fresh = true;
  std::string ret;
...
GenName lower_name(LowerTy lt, const GenName &name,
  ...
  if (lt == LowerTy::DIRECT) {
    ...
    return name_ns_cons(name->with_append_rc("_T"),
                        fmt_str("with_{}", name_aux.as_some()->only()));
    ...
    return name_ns_cons(name->with_append_rc("_T"),
                        fmt_str("is_{}", name_aux.as_some()->only()));
    ...
    return name_ns_cons(name->with_append_rc("_T"),
                        fmt_str("as_{}", name_aux.as_some()->only()));
...
cc::Node_T lower_name_cpp(LowerTy lt, const GenName &name,
  ...
  if (cpp_template_params.is_some()) {
    cc::Node_T ret =
        name_to_cpp_direct(name_low->slice(0, name_low->length() - 1), ctx.cc_);
    if (cpp_template_params.is_some()) {
      ret = ctx.cc_.gen_cpp_id_with_template_args_acc(
          ret, cpp_template_params.as_some());
    }
    ret = cpp_name_with_append(ctx.cc_, ret,
                               name_low->operator[](name_low->length() - 1));
    return ret;
  } else {
...
cc::Node_T data_type_expr_to_cpp(data::Node::Expr_T x, const GenName &ns,
                                 DataCompileContext &ctx) {
  ...
  if (x->is_Id()) {
    ...
    ctx.cc_.Q_->qq_args_acc(lex_args, ">");
    return ctx.cc_.Q_->qq_inner(Some<std::string>("Expr"), lex_args);
...
dt_extract_def_fields_full(const GenName &curr, DataCompileContext &ctx) {
  ...
  auto dt = ctx.data_leaves_->operator[](curr);
  auto fields_full =
      make_rc<Vec<std::pair<IdBase, data::Node::Entry::Field_T>>>();
  auto fields_proper =
      make_rc<Vec<std::pair<IdBase, data::Node::Entry::Field_T>>>();
      ...
      if (data_entry->is_Field()) {
        ...
        if (fields_full_s->contains(data_field_name)) {
          LG_ERR("Field {} is shadowed by datatype {}\n", data_field_name,
                 name_sep_to_underscores(curr));
          AX();
        }
        fields_full_s->insert(data_field_name);
        fields_full->push_back(std::make_pair(data_field_name, cc));
      }
    }
  }
  for (auto data_entry : *dt->entries_) {
    if (data_entry->is_Field()) {
      auto cc = data_entry->as_Field();
      auto data_field_name = cc->name_.to_std_string();
      fields_proper->push_back(std::make_pair(data_field_name, cc));
    }
  }
  return std::make_pair(fields_full, fields_proper);
}
...
dt_extract_def_methods(GenName curr, DataCompileContext &ctx) {
  ...
  for (auto data_entry : *dt->entries_) {
    if (data_entry->is_Method()) {
      ...
      methods->push_back(std::make_pair(method_name, cc));
    }
  }
  return methods;
}
...
Option_T<cc::Node_T> lower_xform_field_entry(
  ...
  if (ty->is_Id()) {
      ...
      if (is_indirect) {
        ...
        if (xform_ty == XformTy::Xform) {
          auto ret = ctx.cc_.gen_cpp_decl_var_init(
              dst, gen_ns,
              lower_name_cpp(LowerTy::STRUCT_RC_ALIAS, name_full, ctx), "y",
              xform_ty_gen_cpp_fn_name(xform_ty, star, top_ty), "(", src, ",",
              src_f, ")");
          return Some<cc::Node_T>(ret);
        } else {
          ctx.cc_.qq_stmt_acc(dst,
                              xform_ty_gen_cpp_fn_name(xform_ty, star, top_ty),
                              "(", src, ",", src_f, ");");
    ...
    auto xc = ty->as_App();
    if (!xc->f_->is_Id()) {
      LG_ERR("Multiple template invocations not supported");
      AX();
    }
    ...
    if (val_hash(f) == val_hash(name_lit({"Vec"}))) {
      if (xc->args_->length() != 1) {
        LG_ERR("Template constructor `Vec` takes exactly one argument");
        AX();
      }
      ...
      auto loop_ind =
          ctx.cc_.gen_cpp_decl_var_init(dst, gen_ns, "langcc::Int", "i", "0");
      auto loop_body = make_rc<Vec<cc::Node_T>>();
      auto ret_i = lower_xform_field_entry(
          xform_ty, star, curr, vis, loop_body,
      ...
      auto loop =
          ctx.cc_.qq("Stmt", "for (", loop_ind, "= 0; ", loop_ind, "_LT_", src,
                     "->length(); ", loop_ind, "++) {", *loop_body, "}");
      dst->push_back(loop);
      return ret;

    } else if (val_hash(f) == val_hash(name_lit({"Option"}))) {
      if (xc->args_->length() != 1) {
        LG_ERR("Template constructor `Option` takes exactly one argument");
        AX();
      }
      auto arg = xc->args_->operator[](0);
      auto arg_ty = data_type_expr_to_cpp(arg, src_ns, ctx);
      ...
      auto ret_i =
          lower_xform_field_entry(xform_ty, star, curr, vis, if_body,
                                  ctx.cc_.qq("Expr", src, ".as_some()"), src_f,
                                  arg, src_ns, gen_ns, header_mode, ctx);

      if (xform_ty == XformTy::Xform) {
        if_body->push_back(ctx.cc_.qq("Stmt", ret.as_some(), "= langcc::Some<",
                                      arg_ty, ">(", ret_i.as_some(), ");"));
      }

      dst->push_back(
          ctx.cc_.qq("Stmt", "if (", src, ".is_some()) {", *if_body, "}"));
      return ret;
...
inline Vec_T<cc::Node_T> inline_maybe_mods(HeaderMode header_mode,
                                           DataCompileContext &ctx) {
...
void data_gen_xform_fn(XformTy xform_ty, const GenName &star,
  ...
  auto cpp_struct_decl_ptr_name_star =
      lower_name_cpp(LowerTy::STRUCT_RC_ALIAS, star, ctx);
  auto cpp_struct_decl_ptr_name_curr =
      lower_name_cpp(LowerTy::STRUCT_RC_ALIAS, curr, ctx);

  auto id_xform_fun_ns = name_lit({
      ctx.cc_.gen_id_fresh(name_lit({}), "__anon__"),
  });
  auto id_xform_name_full =
      name_lit({xform_ty_gen_cpp_fn_name(xform_ty, star, true)});
  ...
  auto cpp_xform_param_f_ty =
      ctx.cc_.qq("Expr", "std::function<", cpp_struct_decl_ptr_name_star, "(",
                 cpp_struct_decl_ptr_name_star, ")>");
  if (xform_ty == XformTy::Visit) {
    cpp_xform_param_f_ty = ctx.cc_.qq_expr("std::function<void(",
                                           cpp_struct_decl_ptr_name_star, ")>");
  }
  ...
  if (is_sum) {
    ...
    for (const auto &[sum_case, sum_case_name] :
         *ctx.sum_cases_->operator[](curr)) {
      ...
      cpp_switch_cases->push_back(
          ctx.cc_
              .qq("SwitchCase", "case",
                  lower_name_cpp(LowerTy::WHICH_ENUM, curr, ctx),
                  "::", sum_case, ": {", *switch_case_body, "break; }")
              ->as_SwitchCase());
    }
    cpp_switch_cases->push_back(
        ctx.cc_.qq("SwitchCase", "default: { langcc::AX(); }"));
    cpp_xform_body->push_back(ctx.cc_.qq("Stmt", "switch (",
                                         cpp_xform_param_x_var, "->w_", ") {",
                                         *cpp_switch_cases, "}"));
    ...
    for (auto [__, q] : *fields_full) {
      ...
      auto cpp_field_item = lower_xform_field_entry(
          ...
          cpp_xform_param_f_var, field_type, curr, id_xform_fun_ns, header_mode,
          ctx);
    ...
    if (xform_ty == XformTy::Xform) {
      ...
      cpp_v = ctx.cc_.gen_cpp_decl_var_init(cpp_xform_body, id_xform_fun_ns,
                                            cpp_struct_decl_ptr_name_curr, "z",
                                            cpp_make_call);
    } else if (xform_ty == XformTy::Visit) {
      cpp_v = cpp_xform_param_x_var;
    }
  ...
  ctx.cc_.dst_decls_->push_back(
      ctx.cc_
          .gen_cpp_fun_proto_decl(cpp_template_params, NodeV_empty(), ret_ty,
  ...
  ctx.cc_.push_def(cpp_template_params->length() > 0,
                   ctx.cc_
                       .gen_cpp_fun_body(cpp_template_params, NodeV_empty(),
                                         ...
                                         ret_ty, cpp_xform_name_full,
                                         cpp_xform_params, NodeV_empty(),
                                         cpp_xform_body)
                       ->as_Decl());
}
...
void data_gen_xform_id_fn(const GenName &star, const GenName &curr,
  ...
  auto id_xform_name_full = name_lit({
      xform_ty_gen_cpp_fn_name(XformTy::Xform, star, false),
  });
  auto cpp_xform_name_full = name_to_cpp_direct(id_xform_name_full, ctx.cc_);
  auto cpp_xform_params = make_rc<Vec<cc::Node_T>>();
  auto cpp_xform_body = make_rc<Vec<cc::Node_T>>();
...
DataDefsResult compile_data_defs(lang::data::Node_T src,
    ...
    ctx.cc_.dst_defs_->push_back(
        ctx.cc_.Q_
            ->qq_ext(Some<std::string>("Decl"),
                     fmt_str("#include \"{}\"", header_name.as_some()))
            ->as_Decl());
  }
  ...
  for (const auto &p : *ctx.enum_leaves_) {
    ...
    for (const auto &data_case : *p.second->cases_) {
      cpp_enum_cases->push_back(
          ctx.cc_.gen_cpp_id_base(data_case.to_std_string()));
    }
    ...
    // hash_ser()
    auto id_hash_ser_fun_ns = name_lit({
        ctx.cc_.gen_id_fresh(name_lit({}), "__anon__"),
    });
    auto id_hash_ser_name_full = name_lit({
        "langcc",
        "hash_ser",
    });
    auto cpp_hash_ser_name_full =
        name_to_cpp_direct(id_hash_ser_name_full, ctx.cc_);
    auto cpp_hash_ser_params = make_rc<Vec<cc::Node_T>>();
    auto cpp_hash_ser_body = make_rc<Vec<cc::Node_T>>();
    auto id_hash_ser_param_buf =
        ctx.cc_.gen_id_fresh(id_hash_ser_fun_ns, "buf");
    auto cpp_hash_ser_param_buf_var =
        ctx.cc_.gen_cpp_id_base(id_hash_ser_param_buf);
    auto cpp_hash_ser_param_buf =
        ctx.cc_.Q_->qq_ext(Some<std::string>("Param"), "langcc::SerBuf&",
    ...
    auto cpp_hash_ser_param_x_var =
        ctx.cc_.gen_cpp_id_base(id_hash_ser_param_x);
    auto cpp_hash_ser_param_x = ctx.cc_.Q_->qq_ext(
        Some<std::string>("Param"), name_to_cpp_direct(name_full, ctx.cc_),
        cpp_hash_ser_param_x_var);
    cpp_hash_ser_params->push_back(cpp_hash_ser_param_x);
    cpp_hash_ser_body->push_back(ctx.cc_.Q_->qq_ext(
        Some<std::string>("Stmt"), "langcc::hash_ser(",
        cpp_hash_ser_param_buf_var, ",", "static_cast<langcc::Int>(",
        cpp_hash_ser_param_x_var, "));"));

    ctx.cc_.dst_decls_->push_back(
        ctx.cc_
            .gen_cpp_fun_proto_decl(NodeV_empty(), NodeV_empty(),
                                    Some(ctx.cc_.gen_cpp_id_base("void")),
                                    id_hash_ser_name_full, cpp_hash_ser_params)
            ->as_Decl());

    ctx.cc_.dst_defs_->push_back(
        ctx.cc_
            .gen_cpp_fun_body(NodeV_empty(), NodeV_empty(),
                              inline_maybe_mods(header_mode, ctx),
                              Some(ctx.cc_.gen_cpp_id_base("void")),
                              cpp_hash_ser_name_full, cpp_hash_ser_params,
                              NodeV_empty(), cpp_hash_ser_body)
            ->as_Decl());

    // pr_debug()
    auto id_pr_debug_fun_ns = name_lit({
        ctx.cc_.gen_id_fresh(name_lit({}), "__anon__"),
    });
    auto id_pr_debug_name_full = name_lit({
        "langcc",
        "pr_debug",
    });
    auto cpp_pr_debug_name_full =
        name_to_cpp_direct(id_pr_debug_name_full, ctx.cc_);
    ...
    auto cpp_pr_debug_param_x_var = ctx.cc_.gen_cpp_param_acc<cc::Node_T>(
        cpp_pr_debug_params, id_pr_debug_fun_ns,
        name_to_cpp_direct(name_full, ctx.cc_), "x");
    auto cpp_switch_cases = make_rc<Vec<cc::Node_T>>();
    for (const auto &data_case : *p.second->cases_) {
      cpp_switch_cases->push_back(
          ctx.cc_
              .qq("SwitchCase", "case", name_to_cpp_direct(name_full, ctx.cc_),
                  "::", data_case.to_std_string(), ": {", "os << ",
                  fmt_str("\"{}\"", data_case.to_std_string()), ";", "break; }")
              ->as_SwitchCase());
    }
    cpp_switch_cases->push_back(
        ctx.cc_.qq("SwitchCase", "default: { langcc::AX(); }"));
    cpp_pr_debug_body->push_back(ctx.cc_.qq("Stmt", "switch (",
    ...
    ctx.cc_.dst_decls_->push_back(
        ctx.cc_
            .gen_cpp_fun_proto_decl(NodeV_empty(), NodeV_empty(),
                                    Some(ctx.cc_.gen_cpp_id_base("void")),
                                    id_pr_debug_name_full, cpp_pr_debug_params)
            ->as_Decl());
    ...
    ctx.cc_.dst_defs_->push_back(
        ctx.cc_
            .gen_cpp_fun_body(NodeV_empty(), NodeV_empty(),
                              inline_maybe_mods(header_mode, ctx),
                              Some(ctx.cc_.gen_cpp_id_base("void")),
                              cpp_pr_debug_name_full, cpp_pr_debug_params,
                              NodeV_empty(), cpp_pr_debug_body)
            ->as_Decl());
  }
  ...
  for (const auto &p : *ctx.data_leaves_) {
    ...
    // struct E;
    ctx.cc_.dst_decls_->push_back(
        ctx.cc_.gen_cpp_decl_struct_fwd(struct_decl_name, cpp_template_params)
            ->as_Decl());
  ...
  auto data_leaves_vec =
      make_rc<Vec<std::pair<GenName, data::Node::Decl::Data_T>>>();
  for (const auto &p : *ctx.data_leaves_) {
  ...
  for (auto [_, i] : data_leaves_inds) {
    ...
    auto [fields_full, fields_proper] =
        dt_extract_def_fields_full(p.first, ctx);
    ...
    if (is_sum) {
      ...
      auto cpp_ctor_param = ctx.cc_.Q_->qq_ext(Some<std::string>("Param"),
                                               cpp_type, cpp_ctor_param_var);
      cpp_ctor_params->push_back(cpp_ctor_param);
      cpp_ctor_body->push_back(ctx.cc_.Q_->qq_ext(
          Some<std::string>("Stmt"), "w_ = ", cpp_ctor_param_var, ";"));
    }
    if (ctx.sum_cases_rev_->contains_key(name_full)) {
      auto [parent_name_full, sum_case_which] =
          ctx.sum_cases_rev_->operator[](name_full);
      auto cpp_parent_which_enum =
          lower_name_cpp(LowerTy::WHICH_ENUM, parent_name_full, ctx);
      auto cpp_struct_decl_name_tmpl_parent =
          cpp_get_templated_name(parent_name_full, LowerTy::STRUCT, ctx);
      cpp_ctor_init_exprs->push_back(
          ctx.cc_.qq("Expr", cpp_struct_decl_name_tmpl_parent, "(",
    ...
    // make_ext()
    auto id_make_ext_fun_ns = name_lit({
        ctx.cc_.gen_id_fresh(name_lit({}), "__anon__"),
    });
    ...
    auto cpp_make_ext_ret = ctx.cc_.gen_cpp_decl_var_init(
        cpp_make_ext_body, cpp_make_name_full, "auto", "ret",
        "langcc::make_rc_ext<", cpp_struct_decl_name_tmpl, ">(",
    ...
    for (auto q : *fields_full) {
      ...
      cpp_with_ret.insert(field_name, ctx.cc_.gen_cpp_decl_var_init(
                                          cpp_with_body[field_name],
                                          cpp_with_name_full[field_name],
                                          "auto", "ret", "langcc::make_rc<",
                                          cpp_struct_decl_name_tmpl, ">()"));
    }
    ...
    if (is_sum) {
      for (const auto &[sum_case, _] : *ctx.sum_cases_->operator[](name_full)) {
        cpp_sum_is_name_full.insert(sum_case,
                                    lower_name(LowerTy::SUM_IS_FULL, name_full,
                                               Some(name_lit({
                                                   sum_case,
                                               }))));
    ...
    if (is_sum) {
      for (const auto &[sum_case, _] : *ctx.sum_cases_->operator[](name_full)) {
        cpp_sum_as_name_full.insert(sum_case,
                                    lower_name(LowerTy::SUM_AS_FULL, name_full,
                                               Some(name_lit({
                                                   sum_case,
                                               }))));
    ...
    // match()
    auto id_sum_match_fun_ns = name_lit({
        ctx.cc_.gen_id_fresh(name_lit({}), "__anon__"),
    });
    GenName cpp_sum_match_name_full =
        lower_name(LowerTy::MATCH_FULL, name_full);
    ...
    if (is_sum) {
      for (const auto &[sum_case, sub_name_full] :
           *ctx.sum_cases_->operator[](name_full)) {
    ...
    auto id_sum_match_expr_fun_type_param =
        ctx.cc_.gen_id_fresh(id_sum_match_expr_fun_ns, "RetT");
    GenName cpp_sum_match_expr_name_full =
        lower_name(LowerTy::MATCH_EXPR_FULL, name_full);
    ...
    if (is_sum) {
        ...
        cpp_switch_cases->push_back(
            ctx.cc_
                .qq("SwitchCase", "case",
                    ...
                    "::", sum_case, ": {", "langcc::pr_debug(",
                    cpp_pr_debug_param_os_var, ",",
                    cpp_pr_debug_param_flags_var, ",", cpp_pr_debug_param_x_var,
                    "->",
      ...
      cpp_switch_cases->push_back(
          ctx.cc_.qq("SwitchCase", "default: { langcc::AX(); }"));
      cpp_pr_debug_body->push_back(ctx.cc_.qq("Stmt", "switch (",
                                              cpp_pr_debug_param_x_var, "->w_",
                                              ") {", *cpp_switch_cases, "}"));
    } else {
      cpp_pr_debug_body->push_back(ctx.cc_.qq(
          "Stmt", "os << ",
      ...
      if (fields_full->length() == 0) {
        ...
        auto flags_sub =
            ctx.cc_.qq("Expr", cpp_pr_debug_param_flags_var, ".sub_lo()");
        for (const auto &q : *fields_full) {
          ...
          cpp_pr_debug_body->push_back(
              ctx.cc_.qq("Stmt", "langcc::pr_debug(", cpp_pr_debug_param_os_var,
                         ",", flags_sub, ",", cpp_pr_debug_param_x_var, "->",
                         fmt_str("{}_", field_name), ");"));
          cpp_pr_debug_body->push_back(ctx.cc_.qq("Stmt", "os << \",\";"));
        }
        cpp_pr_debug_body->push_back(
            ctx.cc_.qq("Stmt", cpp_pr_debug_param_flags_var,
                       ".advance_lines(1, ", cpp_pr_debug_param_os_var, ");"));
        cpp_pr_debug_body->push_back(ctx.cc_.qq("Stmt", "os << \"}\";"));
      }
    }
    ...
    for (auto [_, method] : *methods) {
      ...
      cpp_fields->push(
          ctx.cc_.Q_->qq_inner(Some<std::string>("Entry"), lex_args));
    }

    // hash_ser_acc()
    auto id_hash_ser_acc_fun_ns = name_lit({
        ctx.cc_.gen_id_fresh(name_lit({}), "__anon__"),
    });
    auto hash_ser_acc_name_full =
        lower_name(LowerTy::HASH_SER_ACC_FULL, name_full);
    auto cpp_hash_ser_acc_name_base =
        lower_name_cpp(LowerTy::HASH_SER_ACC_BASE, name_full, ctx);
    auto cpp_hash_ser_acc_name_full =
    ...
    auto hash_ser_acc_inst_name_full =
        lower_name(LowerTy::HASH_SER_ACC_INST_FULL, name_full);
    auto cpp_hash_ser_acc_inst_name_base =
        lower_name_cpp(LowerTy::HASH_SER_ACC_INST_BASE, name_full, ctx);
    auto cpp_hash_ser_acc_inst_name_full =
        lower_name_cpp(LowerTy::HASH_SER_ACC_INST_FULL, name_full, ctx);
    auto cpp_hash_ser_acc_inst_name_full_tmpl = lower_name_cpp(
        LowerTy::HASH_SER_ACC_INST_FULL, name_full, ctx, None<GenName>(),
    ...
    auto cpp_hash_ser_acc_inst_param_buf_var =
        ctx.cc_.gen_cpp_param_acc<cc::Node_T>(
    ...
    if (is_sum) {
      ...
      ctx.cc_.dst_decls_->push_back(
          ctx.cc_
              .gen_cpp_decl_enum(lower_name(LowerTy::WHICH_ENUM, name_full),
                                 cpp_enum_cases)
              ->as_Decl());
      ...
      // is_X()
      for (const auto &[sum_case, _] : *ctx.sum_cases_->operator[](name_full)) {
        cpp_sum_is_body[sum_case]->push_back(ctx.cc_.gen_cpp_ret(
            ctx.cc_.Q_->qq_ext(Some<std::string>("Expr"),
                               "w_ == ", cpp_which_enum, "::", sum_case)));
      }
      ...
      for (const auto &[sum_case, sub_name_full] :
        ...
        auto cpp_struct_decl_name_tmpl_sub =
            cpp_get_templated_name(sub_name_full, LowerTy::STRUCT, ctx);
        auto cpp_sum_is_method_name =
            lower_name_cpp(LowerTy::SUM_IS_BASE, name_full, ctx,
                           Some(name_lit({
                               sum_case,
                           })));
        cpp_sum_as_body[sum_case]->push_back(
            ctx.cc_.Q_->qq_ext(Some<std::string>("Stmt"), "langcc::AT(this->",
                               cpp_sum_is_method_name, "());"));
        cpp_sum_as_body[sum_case]->push_back(
            ctx.cc_.qq_stmt("return this->rc_from_this_poly<",
                            cpp_struct_decl_name_tmpl_sub, ">();"));
      }
      ...
      // match()
      {
          ...
          cpp_switch_cases->push_back(
              ctx.cc_
                  .qq("SwitchCase", "case",
                      lower_name_cpp(LowerTy::WHICH_ENUM, name_full, ctx),
                      "::", sum_case, ": {",
                      cpp_sum_match_params_fs->operator[](i), "(this->",
                      lower_name_cpp(LowerTy::SUM_AS_BASE, name_full, ctx,
                                     Some(name_lit({
                                         sum_case,
                                     }))),
        ...
        cpp_switch_cases->push_back(
            ctx.cc_.qq("SwitchCase", "default: { langcc::AX(); }"));
        cpp_sum_match_body->push_back(ctx.cc_.qq(
            "Stmt", "switch (", "this->w_", ") {", *cpp_switch_cases, "}"));
      }
      ...
      // match_expr()
      {
          ...
          cpp_switch_cases->push_back(
              ctx.cc_
                  .qq("SwitchCase", "case",
                      lower_name_cpp(LowerTy::WHICH_ENUM, name_full, ctx),
                      "::", sum_case, ": {", "return",
                      cpp_sum_match_expr_params_fs->operator[](i), "(this->",
                      lower_name_cpp(LowerTy::SUM_AS_BASE, name_full, ctx,
                                     Some(name_lit({
                                         sum_case,
                                     }))),
        ...
        cpp_switch_cases->push_back(
            ctx.cc_.qq("SwitchCase", "default: { langcc::AX(); }"));
        cpp_sum_match_expr_body->push_back(ctx.cc_.qq(
            "Stmt", "switch (", "this->w_", ") {", *cpp_switch_cases, "}"));
      }
    ...
    if (!has_mod_mut(p.second)) {
      ...
      if (ctx.sum_cases_rev_->contains_key(name_full)) {
        ...
        cpp_hash_ser_acc_inst_body->push_back(
            ctx.cc_.Q_->qq_ext(Some<std::string>("Stmt"), "this->",
                               cpp_hash_ser_acc_inst_parent_name_base, "(",
                               cpp_hash_ser_acc_inst_param_buf_var, ");"));
      }
      if (is_sum) {
        cpp_hash_ser_acc_inst_body->push_back(
            ctx.cc_.Q_->qq_ext(Some<std::string>("Stmt"), "hash_ser(",
                               cpp_hash_ser_acc_inst_param_buf_var,
      ...
      if (!is_sum) {
        cpp_hash_ser_acc_body->push_back(
            ctx.cc_.Q_->qq_ext(Some<std::string>("Stmt"), "this->",
                               cpp_hash_ser_acc_inst_name_full_tmpl, "(",
    ...
    for (const auto &q : *fields_full) {
      ...
      // make()
      auto id_make_param =
          ctx.cc_.gen_id_fresh(cpp_make_name_full, data_field_name);
      auto cpp_make_param_var = ctx.cc_.gen_cpp_id_base(id_make_param);
      auto cpp_make_param = ctx.cc_.Q_->qq_ext(Some<std::string>("Param"),
                                               cpp_type, cpp_make_param_var);
      cpp_make_params->push_back(cpp_make_param);
      cpp_make_body->push_back(
          ctx.cc_.Q_->qq_ext(Some<std::string>("Stmt"), cpp_make_ret, "->",
                             cpp_field_name, " = ", cpp_make_param_var, ";"));

      // make_ext()
      auto id_make_ext_param =
          ctx.cc_.gen_id_fresh(cpp_make_ext_name_full, data_field_name);
      auto cpp_make_ext_param_var = ctx.cc_.gen_cpp_id_base(id_make_ext_param);
      auto cpp_make_ext_param = ctx.cc_.Q_->qq_ext(
          Some<std::string>("Param"), cpp_type, cpp_make_ext_param_var);
      cpp_make_ext_params->push_back(cpp_make_ext_param);
      cpp_make_ext_body->push_back(ctx.cc_.Q_->qq_ext(
          Some<std::string>("Stmt"), cpp_make_ext_ret, "->", cpp_field_name,
          " = ", cpp_make_ext_param_var, ";"));
      ...
      auto cpp_with_param = ctx.cc_.Q_->qq_ext(Some<std::string>("Param"),
      ...
      for (auto data_field_with_rem : *fields_full) {
        ...
        if (data_field_with_rem_name == data_field_name) {
          cpp_with_body[data_field_name]->push_back(ctx.cc_.Q_->qq_ext(
              Some<std::string>("Stmt"), cpp_with_ret[data_field_name], "->",
              cpp_field_with_rem_name, " = ",
              cpp_with_param_vars[data_field_name]->only(), ";"));
        } else {
          cpp_with_body[data_field_name]->push_back(ctx.cc_.Q_->qq_ext(
              Some<std::string>("Stmt"), cpp_with_ret[data_field_name], "->",
              cpp_field_with_rem_name, " = this->", cpp_field_with_rem_name,
              ";"));
        }
      }
      ...
      if (!has_mod_mut(p.second) && !q.second->no_hash__) {
        // hash_ser_acc_E()
        cpp_hash_ser_acc_inst_body->push_back(ctx.cc_.Q_->qq_ext(
            Some<std::string>("Stmt"), "hash_ser(",
            cpp_hash_ser_acc_param_buf_var, ", this->", cpp_field_name, ");"));
      }
    }
    ...
    cpp_fields->push_back(ctx.cc_.gen_cpp_fun_proto_entry(
        NodeV_empty(), NodeV_empty(), None<cc::Node_T>(),
        ctx.cc_.gen_cpp_id_with_template_args_acc(
      ...
      ctx.cc_.dst_decls_->push_back(
          ctx.cc_
              .gen_cpp_fun_proto_decl(
                  cpp_template_params, mods,
                  Some(ctx.cc_.gen_cpp_id_with_template_args_acc(
                      lower_name_cpp(LowerTy::STRUCT_RC_ALIAS, name_full, ctx),
                      cpp_template_params)),
                  lower_name(LowerTy::MAKE_FULL, name_full), cpp_make_params)
              ->as_Decl());
    ...
    if (is_sum) {
      ...
      for (const auto &[sum_case, _] : *ctx.sum_cases_->operator[](name_full)) {
        cpp_fields->push_back(ctx.cc_.gen_cpp_fun_proto_entry(
            NodeV_empty(), NodeV_empty(), Some(ctx.cc_.gen_cpp_id_base("bool")),
            lower_name_cpp(LowerTy::SUM_IS_BASE, name_full, ctx,
                           Some(name_lit({
                               sum_case,
                           }))),
    ...
    if (!has_mod_mut(p.second)) {
      // hash_ser_acc_E()
      cpp_fields->push_back(ctx.cc_.qq(
          "Entry", "void",
          lower_name_cpp(LowerTy::HASH_SER_ACC_INST_BASE, name_full, ctx),
          "(langcc::SerBuf& ", cpp_hash_ser_acc_inst_param_buf_var,
          ") const;"));
      ...
      if (!is_sum) {
        ...
        cpp_fields->push_back(
            ctx.cc_.Q_->qq_ext(Some<std::string>("Entry"),
    ...
    if (!ctx.sum_cases_rev_->contains_key(name_full)) {
      ...
      cpp_base_classes->push_back(
          ctx.cc_.qq_expr("langcc::enable_rc_from_this_poly"));
    }

    ctx.cc_.dst_decls_->push_back(
        ctx.cc_
            .gen_cpp_decl_struct(struct_decl_name, cpp_fields, cpp_base_classes,
                                 cpp_template_params)
            ->as_Decl());
    ...
    ctx.cc_.push_def(
        ...
        ctx.cc_
            .gen_cpp_fun_body(
                ...
                inline_maybe_mods(header_mode, ctx), None<cc::Node_T>(),
                cpp_name_with_append(
                    ctx.cc_,
                    ctx.cc_.gen_cpp_id_with_template_args_acc(
                        cpp_name_with_append(
                            ctx.cc_, name_to_cpp_direct(name_full, ctx.cc_),
                            "_T"),
                        cpp_template_params),
                    "_T"),
                cpp_ctor_params, cpp_ctor_init_exprs, cpp_ctor_body)
            ->as_Decl());
    ...
    if (is_sum) {
      ...
      ctx.cc_.push_def(
          cpp_template_params->length() > 0,
          ctx.cc_.Q_->qq_inner(Some<std::string>("Decl"), lex_args)->as_Decl());
    }
      ...
      ctx.cc_.push_def(
          ...
          ctx.cc_
              .gen_cpp_fun_body(
                  ...
                  cpp_make_params, NodeV_empty(), cpp_make_body)
              ->as_Decl());
      ...
      ctx.cc_.push_def(
          ...
          ctx.cc_
              .gen_cpp_fun_body(
                  ...
                  cpp_make_ext_params, NodeV_empty(), cpp_make_ext_body)
              ->as_Decl());
        ...
        ctx.cc_.push_def(
            ...
            ctx.cc_
                .gen_cpp_fun_body(
                    ...
                    inline_maybe_mods(header_mode, ctx),
                    Some(ctx.cc_.gen_cpp_id_with_template_args_acc(
                        lower_name_cpp(LowerTy::STRUCT_RC_ALIAS, name_full,
                                       ctx),
                        cpp_template_params)),
                    cpp_name_with_append(
                        ctx.cc_,
                    ...
                    cpp_with_params[data_field_name], NodeV_empty(),
                    cpp_with_body[data_field_name])
                ->as_Decl());
      }
    }
    ...
    if (is_sum) {
        ...
        ctx.cc_.push_def(
            ...
            ctx.cc_
                .gen_cpp_fun_body(
                    ...
                    lower_name_cpp(
                        LowerTy::SUM_IS_FULL, name_full, ctx,
                        Some(name_lit({
                            sum_case,
                        })),
                        Some<Vec_T<cc::Node_T>>(cpp_template_params)),
      ...
      for (const auto &[sum_case, sub_name_full] :
           *ctx.sum_cases_->operator[](name_full)) {
        auto cpp_template_params_sub =
            cpp_tabulate_template_params(sub_name_full, ctx);
        auto cpp_struct_decl_name_sub =
            lower_name_cpp(LowerTy::STRUCT_RC_ALIAS, sub_name_full, ctx);
        auto cpp_struct_decl_name_tmpl_sub =
          ...
          ctx.cc_
              .gen_cpp_fun_body(
                  ...
                  lower_name_cpp(LowerTy::MATCH_FULL, name_full, ctx,
                                 None<GenName>(),
                                 Some<Vec_T<cc::Node_T>>(cpp_template_params)),
                  cpp_sum_match_params, NodeV_empty(), cpp_sum_match_body)
              ->as_Decl());
          ...
          ctx.cc_
              .gen_cpp_fun_body(
                  ...
                  lower_name_cpp(LowerTy::MATCH_EXPR_FULL, name_full, ctx,
                                 None<GenName>(),
                                 Some<Vec_T<cc::Node_T>>(cpp_template_params)),
                  cpp_sum_match_expr_params, NodeV_empty(),
                  cpp_sum_match_expr_body)
              ->as_Decl());
    }
      ...
      ctx.cc_.push_def(
          ...
          ctx.cc_
              .gen_cpp_fun_body(
                  ...
                  inline_maybe_mods(header_mode, ctx),
                  Some<cc::Node_T>(
                      name_to_cpp_direct(name_lit({"void"}), ctx.cc_)),
                  lower_name_cpp(LowerTy::HASH_SER_ACC_INST_FULL, name_full,
                                 ctx, None<GenName>(),
                                 Some<Vec_T<cc::Node_T>>(cpp_template_params)),
                  cpp_hash_ser_acc_inst_params, NodeV_empty(),
                  cpp_hash_ser_acc_inst_body, true)
              ->as_Decl());
        ...
        ctx.cc_.push_def(cpp_template_params->length() > 0,
                         ctx.cc_
                             .gen_cpp_fun_body(
                                 cpp_template_params, NodeV_empty(),
                                 inline_maybe_mods(header_mode, ctx),
                                 Some<cc::Node_T>(name_to_cpp_direct(
                                     name_lit({"void"}), ctx.cc_)),
                                 lower_name_cpp(LowerTy::HASH_SER_ACC_FULL,
                                                name_full, ctx, None<GenName>(),
                                                Some<Vec_T<cc::Node_T>>(
                                                    cpp_template_params)),
                                 cpp_hash_ser_acc_params, NodeV_empty(),
                                 cpp_hash_ser_acc_body, true)
                             ->as_Decl());
      }
    }
  }
  ...
  for (auto [_, i] : data_leaves_inds) {
    ...
    if (has_mod_visit(p.second)) {
      ...
      data_gen_xform_fn(XformTy::Visit, p.first, p.first, vis, header_mode,
                        ctx);
    }
