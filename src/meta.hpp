...
inline meta::Node::Stanza::Tokens_T lang_get_tokens_stanza(meta::Node::Lang_T L) {
...
inline meta::Node::Stanza::Parser_T lang_get_parser_stanza(meta::Node::Lang_T L) {
    for (auto& stanza : *L->stanzas_) {
        if (stanza->is_Parser()) {
...
inline Option_T<meta::Node::Stanza::Test_T> lang_get_test_stanza(meta::Node::Lang_T L) {
...
inline Option_T<meta::Node::Stanza::CompileTest_T> lang_get_compile_test_stanza(
    meta::Node::Lang_T L) {

    Option_T<meta::Node::Stanza::CompileTest_T> ret = None<meta::Node::Stanza::CompileTest_T>();
    for (auto& stanza : *L->stanzas_) {
...
inline Int lang_get_k_default(meta::Node::Lang_T L) {
    ...
    for (auto decl : *parser->decls_) {
        if (decl->is_Prop()) {
            for (auto prop : *decl->as_Prop()->props_) {
                if (prop->is_LRSpec()) {
                    auto prop_k = string_to_int(prop->as_LRSpec()->k_.to_std_string()).as_some();
                    AR_ge(prop_k, 0);
                    ret = prop_k;
                }
            }
...
inline Option_T<vector<Ch>> string_extract_lang_char_seq(string s_raw) {
    AT(s_raw.length() >= 2);
    AT(s_raw[0] == '`');
    AT(s_raw[s_raw.length()-1] == '`');
    string s = s_raw.substr(1, s_raw.length() - 2);
...
inline bool lexer_instr_has_emit_match(meta::Node::LexerInstr_T instr) {
    if (instr->is_Emit()) {
        ...
        instr->is_Push() || instr->is_Pop() || instr->is_PopExtract() ||
        instr->is_PopEmit()) {
        return false;
...
inline bool lexer_instrs_has_emit_match(Vec_T<meta::Node::LexerInstr_T> instrs) {
...
inline meta::Node::ParseExpr_T xform_parse_expr_freshen_rec(
    meta::Node::ParseExpr_T e, Gensym_T gen, TokenBounds bounds) {
    ...
    if (e->is_Unfold()) {
        ...
        auto cc = e->as_Id();
        ret = meta::Node::ParseExpr::Id::make(
            gen->gen(), bounds, e->is_top_, e->sym_, e->attr_, e->first_k_, cc->id__);

    } else if (e->is_Eps()) {
        ret = meta::Node::ParseExpr::Eps::make(
            gen->gen(), bounds, e->is_top_, e->sym_, e->attr_, e->first_k_);
        ...
        ret = meta::Node::ParseExpr::Indent::make(
            gen->gen(), bounds, e->is_top_, e->sym_, e->attr_, e->first_k_);

    } else if (e->is_Dedent()) {
        ret = meta::Node::ParseExpr::Dedent::make(
            gen->gen(), bounds, e->is_top_, e->sym_, e->attr_, e->first_k_);

    } else if (e->is_Newline()) {
        ret = meta::Node::ParseExpr::Newline::make(
            gen->gen(), bounds, e->is_top_, e->sym_, e->attr_, e->first_k_);
        ...
        ret = meta::Node::ParseExpr::List::make(
            gen->gen(), bounds, e->is_top_, e->sym_, e->attr_, e->first_k_,
            cc->ty_, xform_parse_expr_freshen_rec(cc->elem_, gen, bounds),
            cc->num_, xform_parse_expr_freshen_rec(cc->delim_, gen, bounds), cc->end_delim_);
...
inline Option_T<string> parse_expr_extract_write_phase_const_ext(ParseExpr_T e) {
    if (e->is_StrLit()) {
        auto cs = string_extract_lang_char_seq(e->as_StrLit()->s_.to_std_string()).as_some();
        string ret = "";
        for (auto c : cs) {
            ret += utf8_encode(c);
        }
        ...
        auto cs = string_extract_lang_char_seq(e->as_Pass()->s_.to_std_string()).as_some();
        string ret = "";
        for (auto c : cs) {
            ret += utf8_encode(c);
        }
...
inline string parse_expr_extract_write_phase_const(ParseExpr_T e) {
